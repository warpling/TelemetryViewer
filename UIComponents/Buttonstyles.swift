    ...
    func makeBody(configuration: Self.Configuration) -> some View {
        VStack(spacing: 0) {
            configuration.label
                ...
                .background(isSelected ? (customAccentColor ?? Color.telemetryOrange).opacity(0.8) : Color.cardBackground)
                .accentColor(isSelected ? Color.cardBackground : (customAccentColor ?? Color.telemetryOrange))
        }
    }
}
