...
final class InsightRetrievalOperation: AsyncOperation {
    ...
    private let onFinish: (InsightResultWrap) -> Void
    private let onStatusChange: (LoadingState) -> Void
...
class InsightResultService: ObservableObject {
    ...
    func calculate(_ insight: DTOv2.Insight, onStatusChange: @escaping (LoadingState) -> Void, onFinish: @escaping (InsightResultWrap) -> Void) {
        // If the result is already cached, return the cached result
